#ifndef LOG_SURGEON_FFI_PYLOGEVENT_HPP
#define LOG_SURGEON_FFI_PYLOGEVENT_HPP
...
//     // Methods
//     /**
//      * Initializes the underlying data with the given inputs. Since the memory
//      * allocation of PyLogEvent is handled by CPython's allocator, cpp
//      * constructors will not be explicitly called. This function serves as the
//      * default constructor to initialize the underlying metadata. It has to be
//      * manually called whenever creating a new PyLogEvent object through CPython
//      * APIs.
//      * @param log_message
//      * @param timestamp
//      * @param index
//      * @param metadata A PyMetadata instance to bind with the log event (can be
//      * nullptr).
//      * @param formatted_timestamp Formatted timestamp. This argument is not given
//      * by default. It should be given when deserializing the object from a saved
//      * state.
//      * @return true on success.
//      * @return false on failure with the relevant Python exception and error set.
//      */
...
//     /**
//      * Initializes the pointers to nullptr by default. Should be called once the
//      * object is allocated.
//      */
...
//     /**
//      * Binds the given PyMetadata and holds a reference. If `Py_metadata` has been
//      * set already, decrement the reference to discard the old value.
//      * @param metadata
//      */
...
//     /**
//      * Gets the formatted log message represented by the underlying log event. If
//      * a specific timezone is provided, this timezone is used to format the
//      * timestamp. If the timezone is not provided (Py_None), the default formatted
//      * timestamp from the log event is used. In the case where the log event does
//      * not have a cached formatted timestamp, it obtains one using the default
//      * timezone from the metadata (if metadata is present), or defaults to UTC.
...
#endif  // LOG_SURGEON_FFI_PYLOGEVENT_HPP
